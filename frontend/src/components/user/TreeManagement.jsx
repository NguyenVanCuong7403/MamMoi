...
function normalizeTree(raw, effectiveGardenId, effectiveGardenName) {
  ...
  const gardenId = raw?.gardenId || effectiveGardenId;
  const gardenName = raw?.gardenName || effectiveGardenName;
...
export default function TreeManagement() {
  ...
  const [garden, setGarden] = useState(() => {
    // fallback: nếu TREES_ARRAY có sẵn gardenId thì lấy cái đầu
    const firstDemoGardenId = Array.isArray(TREES_ARRAY)
      ? (TREES_ARRAY.find((t) => t.gardenId)?.gardenId || "G-001")
      : "G-001";
    return { id: firstDemoGardenId, name: `Vườn ${firstDemoGardenId}` };
  });
  ...
  // Xác định gardenId & name hiệu lực + chuẩn hoá dữ liệu cây
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const canonicalList = rawList.map((raw) => {
        ...
        const merged = base
          ? { ...raw, ...base, id: base.id ?? String(baseId) }
          : { ...raw };

        return normalizeTree(merged, effectiveId, effectiveName);
      });
...
function GardenWeatherPanel({ garden, gardenInfo, weather }) {
  // Vẫn lấy tên vị trí để dùng cho aria-label (không hiển thị trong UI)
  const locationName =
    (gardenInfo && formatGardenLocation(gardenInfo)) ||
    garden?.name ||
    "Đang tải vị trí...";

  // Demo dữ liệu – sau này thay bằng dữ liệu API
  const demoWeather = {
    current: {
      temp: 29,
      feelsLike: 31,
    ...
    alerts: [
      {
        ...
        severity: "warning", // warning | danger | watch
      },
    ],
    forecast: [
      {
  ...
  // Nếu đã có dữ liệu thật thì dùng, không thì fallback demo
  const data =
 ...
 return (
  <div
    className="w-full rounded-2xl p-3 md:p-3.5 backdrop-blur-md text-white border border-white/15 bg-gradient-to-r from-white/10 via-white/5 to-emerald-400/20 shadow-xl h-[140px] md:h-[150px] flex items-center"
...
<div className="flex-1 min-w-0 flex flex-col gap-1">
  {/* Hàng tiêu đề: Thời tiết vườn + Cập nhật */}
  ...
  <div className="flex items-center gap-3 md:gap-4">
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-baseline flex-wrap gap-x-2 gap-y-0.5">
        ...
        {/* Đường kẻ dọc trên desktop */}
        <div className="hidden md:block w-px bg-white/15" />

         {/* Khối 2: Cảnh báo thời tiết – stretch full height để chạm đáy */}
      {/* Khối 2: Cảnh báo thời tiết (không còn icon tam giác) */}
{/* Khối 2: Cảnh báo thời tiết (pill là icon) */}
<div className="flex-1 min-w-[220px] flex flex-col">
  <div
    className={
      "rounded-xl px-3 py-2.5 border " + alertStyle.wrapper
    }
  >
    <div className="space-y-0.5">
      ...
      {alerts.length > 1 && (
        ...
        {/* Khối 3: Dự báo 3 mốc gần */}
        {forecast.length > 0 && (
          <div className="mt-1 md:mt-0 grid grid-cols-3 gap-2 w-full md:w-[260px]">
