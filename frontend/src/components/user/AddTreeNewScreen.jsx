...
<div className="col-span-12 md:col-span-6 xl:col-span-3 grid gap-1">
  ...
  <SearchableSelect
    ...
    options={SPECIES_LIST.map((s) => ({ value: s.key, label: s.label }))}
...
<div className="col-span-12 md:col-span-6 xl:col-span-3 grid gap-1">
  ...
  <SearchableSelect
    ...
    options={
      speciesKey
        ? (soilKB?.options || []).map((opt) => ({ value: opt, label: opt }))
        : []
    }
    placeholder={speciesKey ? "— Chọn loại đất —" : "Chọn loại cây trước"}
    disabled={!speciesKey}
    error={errors.soil}
