...
function SoilInfo({ soil, soilName }) {
  if (!soil && !soilName) {
    return (
      <Badge
        variant="secondary"
  ...
  if (!soil) {
    return (
      <Badge
        variant="secondary"
        className="rounded-full bg-emerald-50 px-4 py-1 text-sm text-emerald-600"
      >
        {soilName || "Chưa gán"}
      </Badge>
    );
          ...
          <div>
            <Badge
              ...
              className={cn(
                ...
                hasDetails
                  ? "cursor-help bg-emerald-50 text-emerald-600"
                  : "bg-emerald-50 text-emerald-600"
              )}
            >
              {soil.SoilName || soilName || "Chưa gán"}
            </Badge>
          </div>
        </TooltipTrigger>
        {hasDetails && (
          <TooltipContent className="max-w-xs">
            <div className="space-y-2">
              ...
              <div className="space-y-1.5 text-xs text-slate-600">
                ...
                {soil.Drainage && (
                  <div className="flex items-center gap-2">
                    <span className="font-medium">Thoát nước:</span>
                    <span>{soil.Drainage}</span>
                  </div>
                )}
                {soil.OrganicMatterPct !== null &&
                  soil.OrganicMatterPct !== undefined && (
                    <div className="flex items-center gap-2">
                      <span className="font-medium">Hữu cơ:</span>
                      <span>{soil.OrganicMatterPct}%</span>
                    </div>
                  )}
                {soil.EC_dS_m !== null && soil.EC_dS_m !== undefined && (
                  <div className="flex items-center gap-2">
                    <span className="font-medium">Độ mặn EC:</span>
                    <span>{soil.EC_dS_m} dS/m</span>
                  </div>
                )}
                {soil.Notes && (
                  <div className="mt-2 border-t border-slate-200 pt-2">
                    <p className="text-xs text-slate-500">{soil.Notes}</p>
                  </div>
                )}
              </div>
            </div>
              ...
              >
                ...
                <TableCell className="hidden text-center align-middle md:table-cell">
                  <div className="flex justify-center">
                    <SoilInfo
                      soil={soilLookup[tree.SoilMasterID]}
                      soilName={tree.SoilMasterName}
                    />
                  </div>
                </TableCell>
                <TableCell className="hidden text-center align-middle md:table-cell">
...
export default function TreeTypeManagement() {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      if (editingTree) {
        ...
        if (updated) {
          // Fetch varieties if needed
          if (!updated.Varieties) {
            try {
              const varietiesResponse =
                await AdminTreeRepository.getVarietiesByTreeTypeId(
                  parseInt(updated.TreeTypeID)
                );
              const varieties = Array.isArray(varietiesResponse.data)
                ? varietiesResponse.data
                : Array.isArray(varietiesResponse)
                ? varietiesResponse
                : [];
              updated.Varieties = varieties.map(mapVarietyFromApi);
            } catch (error) {
              console.warn("Failed to fetch varieties:", error);
              updated.Varieties = [];
            }
          }
  ...
  const handleToggleStatus = async (tree) => {
    try {
      ...
      const treeTypeId = parseInt(tree.TreeTypeID);
      const updated = await AdminTreeRepository.updateTreeType(
        treeTypeId,
        apiPayload
      );
      const mappedUpdated = mapTreeTypeFromApi(updated);

      // Fetch varieties if needed
      if (mappedUpdated && !mappedUpdated.Varieties) {
        try {
          const varietiesResponse =
            await AdminTreeRepository.getVarietiesByTreeTypeId(treeTypeId);
          const varieties = Array.isArray(varietiesResponse.data)
            ? varietiesResponse.data
            : Array.isArray(varietiesResponse)
            ? varietiesResponse
            : [];
          mappedUpdated.Varieties = varieties.map(mapVarietyFromApi);
        } catch (error) {
          console.warn("Failed to fetch varieties:", error);
          mappedUpdated.Varieties = [];
        }
      }

      setTrees((prev) =>
        prev.map((item) =>
          item.TreeTypeID === mappedUpdated?.TreeTypeID ? mappedUpdated : item
        )
      );
    } catch (error) {
      console.error("Error toggling tree type status:", error);
      throw error;
    }
  };
  ...
  const handleOpenVarietyDialog = async (tree) => {
    ...
    try {
      ...
      const mappedVarieties = varieties.map(mapVarietyFromApi);
      const updatedTree = {
        ...tree,
        Varieties: mappedVarieties,
      };
      setActiveVarietyTree(updatedTree);
      setSelectedVarietyId(mappedVarieties?.[0]?.VarietyID ?? null);
      varietyDetailForm.reset(
        mapVarietyToFormValues(mappedVarieties?.[0] ?? null)
      );
    } catch (error) {
  ...
  const handleCreateVariety = async (payloadOverride) => {
    ...
    try {
      const apiPayload = {
        treeTypeId: parseInt(activeVarietyTree.TreeTypeID),
        varietyName: values.VarietyName,
        varietyDescription: values.VarietyDescription ?? "",
      };
      const created = await AdminTreeRepository.createTreeVariety(apiPayload);
      const newVariety = mapVarietyFromApi(created);

      // Fetch updated varieties list
      const varietiesResponse =
        await AdminTreeRepository.getVarietiesByTreeTypeId(
          parseInt(activeVarietyTree.TreeTypeID)
        );
      const varieties = Array.isArray(varietiesResponse.data)
        ? varietiesResponse.data
        : Array.isArray(varietiesResponse)
      ...
      const updatedVarieties = varieties.map(mapVarietyFromApi);
      const updatedTree = {
        ...activeVarietyTree,
        Varieties: updatedVarieties,
      };
      syncUpdatedTreeVarieties(updatedTree);
      ...
      setSelectedVarietyId(newVariety?.VarietyID ?? null);
      setVarietyDetailEditMode(false);
      setCreateVarietyOverlayOpen(false);
      setVarietyCreateConfirmOpen(false);
      setPendingVarietyCreate(null);
  ...
  const handleUpdateVariety = async (valuesOverride) => {
    ...
    try {
      ...
      const updated = await AdminTreeRepository.updateTreeVariety(
        varietyId,
        apiPayload
      );
      const updatedVariety = mapVarietyFromApi(updated);
      ...
      setPendingVarietyUpdate(null);
      setVarietyUpdateConfirmOpen(false);
      // Update selected variety if it still exists
      if (updatedVariety) {
        setSelectedVarietyId(updatedVariety.VarietyID);
      }
    } catch (error) {
  ...
  const handleDeleteVariety = async () => {
    ...
    try {
      const varietyId = parseInt(selectedVarietyId);
      await AdminTreeRepository.deleteTreeVariety(varietyId);
