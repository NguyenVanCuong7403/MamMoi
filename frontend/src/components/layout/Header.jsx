...
export default function MMHeader({
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    if (!isAdmin(user)) {
      items.push({ id: "thu-vien-cay", label: "Thư viện Cây", href: "/plants" });
    }
    ...
    <>
      ...
      >
        <div className="w-full px-4 md:px-6 h-[80px] flex items-center gap-5">
          ...
          <a
            href="#"
            className="inline-flex items-center gap-3 focus:outline-none active:outline-none rounded flex-shrink-0"
            onClick={(e) => {
              navigate("/");
            }}
          >
          ...
          <button
            onClick={() => {
              ...
              window.dispatchEvent(new CustomEvent("mm:navigation:force-reload"));
              navigate(-1);
            }}
